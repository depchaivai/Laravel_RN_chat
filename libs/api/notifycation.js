...
lastNotificationId = 0;
  ...
  config = () => {
    PushNotification.configure({
      ...
      onRegister: async (token) => {
        ...
        try {
            await setInternetCredentials('dv_token','dv_token',token.token);
            console.log('da set keychain');
        } catch (error) {
            console.log('khong the set keychain');
        }
   ...
   */
  triggerLocalNotification = (soundName) => {
